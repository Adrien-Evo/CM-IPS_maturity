...
tss_intersect = read.table(file.path(rootfolder,radio$intersect.tss),h=F,sep = "\t")
...
head(tss_intersect.dcast)
tss_intersect.dcast <- tss_intersect.dcast[,c(1,2,4,5,6,3)]
head(tss_intersect.dcast)
...
kmodes_10_all <- kmodes(tss_intersect.dcast[,2:6], 10, iter.max = 20, weighted = TRUE)

kmodes_15_all <- kmodes(tss_intersect.dcast[,2:6], 15, iter.max = 10, weighted = FALSE )

kmodes_20_all <- kmodes(tss_intersect.dcast[,2:6], 20, iter.max = 20, weighted = TRUE )

##########################
######  Individual clustering per cmIPS state
########################
...
tss_intersect.dcast$singular_kmeans = rep(0,length(tss_intersect.dcast[,1]))

for(state in colourcmips$V1){

    kmodes_10_temp = kmodes(tss_intersect.dcast[which(tss_intersect.dcast[,1] == state),2:6],10, iter.max = 5, weighted = FALSE)

    tss_intersect.dcast$singular_kmeans[which(tss_intersect.dcast[,1] == state)] <- kmodes_10_temp$cluster
}
